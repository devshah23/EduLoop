...
@router.get("/all/{class_id}")
async def get_all_students_class(class_id: int, db: AsyncSession = Depends(get_db),_=Depends(require_role(UserTypeEnum.FACULTY))):
    return await student_crud.get_all_students_class(db, class_id)
