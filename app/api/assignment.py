...
@router.get("/all")
async def get_assignments(db: AsyncSession = Depends(get_db),_=Depends(require_role(UserTypeEnum.FACULTY))):
    assignments = await assignment_crud.get_assignments(db)
    return assignments
...
async def update_assignment(assignment_id: int, assignment: AssignmentUpdate, db: AsyncSession = Depends(get_db),_=Depends(require_role(UserTypeEnum.FACULTY))):
    updated = await assignment_crud.update_assignment(db, assignment_id, assignment)
    return updated
