...
from app.schemas.class_schema import ClassCreate, ClassUpdate, ClassRead
...
@router.put("/{class_id}")
async def update_class(class_id: int, class_: ClassUpdate, db: AsyncSession = Depends(get_db),current_user=Depends(require_role(UserTypeEnum.FACULTY))):
...
async def delete_class(class_id: int, db: AsyncSession = Depends(get_db),current_user=Depends(require_role(UserTypeEnum.FACULTY))):
    deleted = await class_crud.delete_class(db, class_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Class not found")